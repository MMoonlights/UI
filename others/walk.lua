...
local function makeBall(pos, color, size)
...
function Walker:_repath(force)
    ...
    task.spawn(function()
        ...
        if not ok or self.path.Status == Enum.PathStatus.NoPath then
            if self:_hasLOS(hrp.Position, self.target) then
                ...
                self.mode = "wait"
                self.failRetries += 1
                if self.failRetries > self.opts.MaxNoPathRetries then
                    self:_finish(false, "no path")
                end
            end
        else
            self.failRetries = 0
...
function Walker:_progressCheck(dt, hrp)
    ...
    if dist < self.bestDist - self.opts.ProgressRequired then
        self.bestDist = dist
        self.stuckTicks = 0
        if self.bestDist < self.startBest - 12 then
            self.startBest = self.bestDist
            self.repathCount = 0
        end
    else
        self.stuckTicks += 1
        if self.stuckTicks >= self.opts.StuckRepathTicks then
            self.stuckTicks = 0
            self:_repath(true)
        elseif self.stuckTicks >= self.opts.StuckJumpTicks then
            local hum = self:_humanoid()
